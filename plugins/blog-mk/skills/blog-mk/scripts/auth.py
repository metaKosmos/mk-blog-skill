#!/usr/bin/env python3
"""
auth.py — Login por email mK para a skill blog-mk.
...
O ID token (NAO uma credencial do WordPress) e enviado ao broker, que valida
o email/dominio antes de publicar. A senha do WordPress nunca chega nesta maquina.
...
TOKEN_CACHE = Path.home() / ".blog-mk-auth.json"
...
class _OAuthCallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802 (interface da stdlib)
        ...
        self.wfile.write(
            ...
            f"<h2>blog-mk</h2><p>{msg}</p></body></html>".encode("utf-8")
        )
...
def main():
    p = argparse.ArgumentParser(description="Login por email mK para a skill blog-mk")
