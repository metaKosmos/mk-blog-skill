"""
config.py — Configuracao publica da skill blog-mk (sem segredos no repo).
...
Google; o gate real de publicacao e a allowlist no broker.
...
def get_oauth_client():
    ...
    Ordem: env vars (BLOG_MK_GOOGLE_CLIENT_ID/SECRET) -> endpoint /oauth-config do
    broker. Buscado de forma preguicosa (so quando o login precisa), entao operacoes
    offline como --dry-run nao dependem de rede.
    """
    global _oauth_cache
    cid = os.environ.get("BLOG_MK_GOOGLE_CLIENT_ID")
    csec = os.environ.get("BLOG_MK_GOOGLE_CLIENT_SECRET")
    if cid and csec:
        return cid, csec
    if _oauth_cache:
        return _oauth_cache
...
BROKER_URL = os.environ.get(
    "BLOG_MK_BROKER_URL",
    "https://blog-broker-lpbyopeg7q-rj.a.run.app",
).rstrip("/")
...
SESSION_MAX_AGE_HOURS = float(os.environ.get("BLOG_MK_SESSION_MAX_AGE_HOURS", "12"))
